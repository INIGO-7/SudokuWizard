...
class SudokuWizard():
    ...
    def scan_image(self, verbose : bool = False) -> (np.ndarray, np.ndarray):
        ...
        cnts = cv.findContours(blurred.copy(), cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
        ...
        sudoku_thresh = four_point_transform(blurred, sudoku_contour.reshape(4, 2))
        ...
        self.sudoku = self.resize_if_large(sudoku, 600, 600)
        self.sudoku_thresh = self.resize_if_large(sudoku_thresh, 600, 600)
    ...
    def extract_cells(self, verbose : bool = False):
        ...
        # We have to scan the image first to get the cropped sudoku, so if sudoku_thresh is none, raise exception.
        ...
        cnt_area = blurred.shape[0] + blurred.shape[1]
        ...
        if verbose:
            cv.imshow("Repasamos lineas horizontales y verticales otra vez", fixed_img)
            cv.waitKey(0)
